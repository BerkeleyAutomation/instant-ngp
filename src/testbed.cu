...
void Testbed::imgui() {
	...
	if (ImGui::CollapsingHeader("Training", m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0)) {
		...
		if (m_testbed_mode == ETestbedMode::Nerf && ImGui::TreeNode("NeRF training options")) {
			...
			ImGui::SliderFloat("Cone angle", &m_nerf.cone_angle_constant, 0.0f, 1.0f/8.0f);
			ImGui::SliderFloat("Min Transmittance", &m_nerf.rendering_min_transmittance, 0.0f, 1.0f);
